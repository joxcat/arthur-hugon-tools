...
impl SimpleDB {
    pub fn init(_path: &str) -> Result<Self, SimpleDBError> {
        let path = Path::new(_path);
        if !path.is_file() {
            File::create(path).map_err(err!("Cannot create file"))?;
        }
        Ok(Self { inner: Mutex::new(_path.to_owned()) })
    }
    ...
    pub fn get(&self, key: &str) -> Result<Option<String>, SimpleDBError> {
        let locked = self.inner.lock().unwrap();
        let mut f = File::open(Path::new(locked.as_str())).map_err(err!("Cannot open file"))?;
